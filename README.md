# Hi, I'm Umar Musa Isah 👋

### Data Analyst | Monitoring & Evaluation (M&E) Professional | Humanitarian Analytics Specialist

![Excel](https://img.shields.io/badge/Excel-Advanced-green)
![SQL](https://img.shields.io/badge/SQL-Intermediate-blue)
![Power BI](https://img.shields.io/badge/PowerBI-Dashboard-yellow)
![M&E](https://img.shields.io/badge/M&E-Professional-orange)

I am a Data Analyst specializing in transforming raw data into actionable insights that support evidence-based decision-making in humanitarian, development, and business environments.

I build dashboards, automate reporting workflows, analyze indicators, and communicate insights clearly to stakeholders and decision-makers.

---

## About Me

- 📊 Data Analyst focused on analytics, reporting, and decision support
- 🌍 Interested in NGO, INGO, and UN-driven data systems
- 📈 Specialized in dashboards, KPI tracking, and M&E reporting
- 🧠 Passionate about solving operational problems with data
- 📍 Open to remote and international opportunities

---

## Core Skills

- Microsoft Excel
- SQL
- Power BI
- Data Cleaning
- Data Visualization
- Dashboard Design
- Reporting
- Monitoring & Evaluation (M&E)
- KoboToolbox
- CommCare
- PowerPoint

---

## Tools & Technologies

| Category | Tools |
|----------|-------|
| Analytics | Excel, SQL, Power BI |
| Survey Tools | KoboToolbox, CommCare, Google Forms |
| Reporting | PowerPoint, PDF Reporting |
| Data Processing | Power Query, Excel Modeling |

---

## Certifications

- Certificate in Data Analytics (Excel, SQL, Power BI)
- Monitoring & Evaluation Professional
- CommCare Certified – Application Building (Dimagi)

---

## Featured Projects

### UNICEF Education Monitoring Dashboard
Tracks enrollment, attendance, gender parity, and intervention performance.

### WFP Food Security Dashboard
Analyzes food insecurity trends and vulnerable populations.

### UNDP M&E Indicator Tracker
Monitors KPI performance and project delivery indicators.

---

## What I Do

✔ Clean messy datasets  
✔ Build dashboards  
✔ Track indicators  
✔ Write analytical reports  
✔ Generate business insights  
✔ Support decision-making  

---

## Contact

- Email: umarmusapress@gmail.com
- GitHub: https://github.com/UmarMusaIsah

---

> “Data becomes valuable when transformed into insight.”
